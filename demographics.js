...
if (
  student_data !== 0 ||
  (typeof student_data === "object" && Object.keys(student_data).length !== 0)
) {
  ...
  const min_age_students = student_data.filter(
    (student) => student.age === min_age
  );
  const min_age_students_str = min_age_students
    .map(
      (student) =>
        student.first + " " + student.last + " (id: " + student.id + ")"
    )
    .join("\n\t\t");
  const max_age = Math.max(...ages);
  const max_age_students = student_data.filter(
    (student) => student.age === max_age
  );
  const max_age_students_str = max_age_students
    .map(
      (student) =>
        student.first + " " + student.last + " (id: " + student.id + ")"
    )
  ...
  const avg_lunch_bal =
    student_data.reduce(
      (total, next) => total + parseInt(next.lunch_balance.replace("$", "")),
      0
    ) / Object.keys(student_data).length;

  console.log(
    `Generated demographic data from ${
      Object.keys(student_data).length
    } students...`
  );
  ...
  console.log(
    `\t The minimum age of the students is ${min_age} years. These students are: \n\t\t${min_age_students_str}`
  );
  console.log(
    `\t The maximum age of the students is ${max_age} years. These students are: \n\t\t${max_age_students_str}`
  );
  console.log(`\t The average age of the students is ${avg_age} years.`);
  console.log(
    `\t The average lunch balance of the students is $${avg_lunch_bal}.`
  );
}
...
if (
  teacher_data !== 0 ||
  (typeof teacher_data === "object" && Object.keys(teacher_data).length !== 0)
) {
  ...
  console.log(
    `\n\nGenerated demographic data from ${
      Object.keys(teacher_data).length
    } teachers...`
  );
  ...
  Object.keys(group_by_subjects).forEach((subject) => {
    ...
    const str = group_by_subjects[subject]
      .map(
        (teacher) =>
          teacher.first + " " + teacher.last + " (id: " + teacher.id + ")"
      )
      .join("\n\t\t");
    console.log(
      `\t The teacher${teachers_num > 1 ? "s" : ""} teaching ${subject} ${
        teachers_num > 1 ? "are" : "is"
      } \n\t\t${str}`
    );
  });
}
